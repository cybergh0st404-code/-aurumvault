...
export async function listAllUsers(): Promise<SafeUserRecord[]> {
  ...
  const db = getDb()
  const res = await db.execute(`
    SELECT id, email, name, role, client_code, organization, avatar_initials, security_clearance, created_at, updated_at
    FROM users
...
export async function getUserBySessionToken(token: string): Promise<UserProfile | null> {
  ...
  const res = await db.execute({
    sql: `
      SELECT u.id, u.email, u.name, u.role, u.client_code, u.organization, u.avatar_initials, u.security_clearance, s.expires_at
      FROM sessions s
